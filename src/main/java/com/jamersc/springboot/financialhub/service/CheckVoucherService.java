...
/*    public ByteArrayInputStream generateCheckVoucher(CheckDto checkDto) {
        ...
        try {
            ...
            // Title
            Paragraph title = new Paragraph("Check Voucher", titleFont);
            ...
            // PCV Number
            Paragraph pcvNumber = new Paragraph("CV No.: " + checkDto.getCvNumber(), regularFont);
            pcvNumber.setAlignment(Element.ALIGN_LEFT);  // Align to the left
            document.add(pcvNumber);

            // Particulars
            Paragraph particulars = new Paragraph("Amount in words: " + checkDto.getAmountInWords(), regularFont);
            particulars.setAlignment(Element.ALIGN_LEFT);
            document.add(particulars);

            // Format the amount to two decimal places
            String formattedAmount = String.format("%.2f", checkDto.getTotalAmount());
            Paragraph amount = new Paragraph("Amount: " + formattedAmount, regularFont);
            amount.setAlignment(Element.ALIGN_LEFT);
            document.add(amount);

            // Received By
            Paragraph receivedBy = new Paragraph("Received By: " + checkDto.getPayeeName(), regularFont);
            receivedBy.setAlignment(Element.ALIGN_LEFT);
            document.add(receivedBy);

            // Approved By
            Paragraph approvedBy = new Paragraph("Approved By: " + checkDto.getPayeeName(), regularFont);
